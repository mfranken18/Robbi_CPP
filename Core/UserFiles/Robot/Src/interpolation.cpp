...
uint32_t micros()
{
	return HAL_GetTick() / HAL_GetTickFreq() / 1000;
}
...
void Interpolation::setPosOffset(float new_x, float new_y, float new_z, float new_e) {
  ...
  Logger::logINFO("POSITION OFFSET: [X" + std::to_string(pos_offset.xmm) + " Y:" + std::to_string(pos_offset.ymm) + " Z:" + std::to_string(pos_offset.zmm) + " E:" + std::to_string(pos_offset.emm) + "]");
  Logger::logINFO("CURRENT POSITION: [X:" + std::to_string(new_x)+" Y:" + std::to_string(new_y)+" Z:" + std::to_string(new_z) + " E:" + std::to_string(new_e) + "]");
}
...
bool Interpolation::isAllowedPosition(float pos_tracker[4]) {
  ...
  float squaredPositionModule = pow(rrot_x, 2) + pow(rrot_y, 2) + pow(pos_tracker[Z_AXIS], 2);

      bool retVal = (
          squaredPositionModule <= pow(R_MAX, 2)
          && squaredPositionModule >= pow(R_MIN, 2)
  ...
  if(!retVal) {
    Logger::logERROR("LIMIT REACHED: [X:" + std::to_string(pos_tracker[X_AXIS]) + " Y:" + std::to_string(pos_tracker[Y_AXIS]) + " Z:" + std::to_string(pos_tracker[Z_AXIS]) + " E:" + std::to_string(pos_tracker[E_AXIS]) + "]");
  }
  return retVal;
}
